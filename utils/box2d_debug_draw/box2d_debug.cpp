...
b2DrawRayLib::b2DrawRayLib(float scale) noexcept : m_scale{scale} {}
...
void b2DrawRayLib::SetScale(float scale) noexcept { m_scale = scale; }

float b2DrawRayLib::GetScale() noexcept { return m_scale; }
...
void b2DrawRayLib::DrawSolidCircle(b2Vec2 const& center, float radius, b2Vec2 const& /* axis */,
...
float b2DrawRayLib::M_ToPixels(float f) const noexcept { return f * m_scale; }

Vector2 b2DrawRayLib::M_ToPixels(b2Vec2 const& p) const noexcept { return {p.x * m_scale, p.y * m_scale}; }

Color b2DrawRayLib::M_ConvertColor(b2Color const& color) const noexcept { return M_ConvertColor(color, color.a); }

Color b2DrawRayLib::M_ConvertColor(b2Color const& color, float newAlpha) const noexcept {
    return {static_cast<unsigned char>(color.r * 255.0f), static_cast<unsigned char>(color.g * 255.0f),
            static_cast<unsigned char>(color.b * 255.0f), static_cast<unsigned char>(newAlpha * 255.0f)};
}